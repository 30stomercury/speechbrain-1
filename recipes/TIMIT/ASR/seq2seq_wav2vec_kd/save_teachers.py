...
class ASR(sb.Brain):
    def compute_forward_tea(self, batch):
        ...
        tea_dict = {}
        with torch.no_grad():
            x_tea = self.modules.enc(feats)
            ctc_logits_tea = self.modules.ctc_lin(x_tea)
            ...
            e_in_tea = self.modules.emb(phns_bos)
            h_tea, _ = self.modules.dec(e_in_tea, x_tea, wav_lens)

            # output layer for seq2seq log-probabilities
            seq_logits_tea = self.modules.seq_lin(h_tea)
            p_seq_tea = self.hparams.log_softmax(
        ...
        for num in range(len(stage)):
            ...
            with tqdm(
                ...
                for batch in t:
                    ...
                    tea_dict = self.compute_forward_tea(batch)
