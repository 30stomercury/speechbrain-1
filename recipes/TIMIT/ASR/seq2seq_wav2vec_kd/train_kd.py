...
class ASR(sb.Brain):
    ...
    def re_format(self, data_dict):
        ...
        tea_name = self.hparams.tea_name
        for i in range(len(self.hparams.tea_keys)):
            item_tea = data_dict[str(self.step)][tea_name][
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        p_ctc_tea, p_seq_tea, wer_ctc_tea, wer_tea = item_tea_list

        # calculate CTC distillation loss
        ctc_loss_kd = self.hparams.ctc_cost_kd(
            p_ctc, p_ctc_tea, wav_lens, device=self.device
        )
        # calculate CE distillation loss
        seq2seq_loss_kd = self.hparams.seq_cost_kd(
            p_seq, p_seq_tea, phn_lens_eos
        )
        # total loss
